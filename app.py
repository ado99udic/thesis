...
def format_output_as_table(output):
    ...
    for line in lines:
        if line.startswith('Route for vehicle'):
            ...
            current_vehicle = {'Vehicle': line.split(' ')[3], 'Route for vehicle': ''}
    ...
    html = '<table>'
    html += '<tr><th>Vozidlo</th><th>Trasa</th><th>Prejdená vzdialenosť</th></tr>'
    for vehicle in data:
        html += f'<tr><td>Vehicle {vehicle["Vehicle"]}</td><td>{vehicle["Route for vehicle"]}</td><td>{vehicle["Distance of the route"]}</td></tr>'
    html += '</table>'
